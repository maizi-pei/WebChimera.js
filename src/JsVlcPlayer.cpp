...
struct JsVlcPlayer::LibvlcLogEvent : public JsVlcPlayer::AsyncData {
    LibvlcLogEvent(int level, const std::string &message, const std::string &format) : level(level), message(message),
                                                                                       format(format) {}
...
JsVlcPlayer::JsVlcPlayer(
        ...
        const v8::Local <v8::Array> &vlcOpts) : _libvlc(nullptr) {
    using namespace v8;
...
void *JsVlcPlayer::onFrameSetup(const RV32VideoFrame &videoFrame) {
    ...
    Local <Integer> jsUOffset = Integer::New(isolate, 0);
    Local <Integer> jsVOffset = Integer::New(isolate, 0);
    ...
    callCallback(CB_FrameSetup, {jsWidth, jsHeight, jsUOffset, jsVOffset, jsPixelFormat, jsArray});
...
void *JsVlcPlayer::onFrameSetup(const I420VideoFrame &videoFrame) {
    ...
    Local <Integer> jsUOffset = Integer::New(isolate, videoFrame.uPlaneOffset());
    Local <Integer> jsVOffset = Integer::New(isolate, videoFrame.vPlaneOffset());
